...
@section('content')
    <div class="dashboard-balancing">
        ...
        <div class="right-item" id="switch-master-ongoing">
            <a class="item {{ request()->machine_type === 'washing' ? 'active' : '' }}"
                href='{{ route("backend.dashboard.on-process.$type.index") . '?factory_id=' . request()->factory_id . '&date1=' . request()->date1 . '&date2=' . request()->date2 . '&machine_type=washing' }}'>Rotary</a>
            <a class="item {{ request()->machine_type === 'tunnel' ? 'active' : '' }}"
                href='{{ route("backend.dashboard.on-process.$type.index") . '?factory_id=' . request()->factory_id . '&date1=' . request()->date1 . '&date2=' . request()->date2 . '&machine_type=tunnel' }}'>Tunnel</a>
        </div>

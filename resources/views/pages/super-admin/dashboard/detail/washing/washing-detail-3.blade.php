...
@section('content')
    <div class="dashboard-balancing">
        <div class="top-page">
            <a
                href="{{ route('backend.dashboard.on-process.washing.index') . '?factory_id=' . request()->factory_id . '&date1=' . request()->date1 . '&date2' . request()->date2 . '&machine_type=' . $type }}"><img
                    src="{{ asset('images/icon/arrow-back.svg') }}" alt="Ironing"></a>
            <p>Detail / <span style="font-weight: 700">CF {{ request()->type == 'ongoing' ? 'Ongoing' : 'Done' }}</span></p>
        </div>
    ...
    <script>
        $(document).ready(function() {
            $('#table-master-user').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: `{{ route("backend.dashboard.on-process.$type.show", ':id') }}`.replace(':id',
                        '{{ $id }}'),
                    type: 'GET',
                    data: function(d) {
                ...
                columns: [{
                    ...
                    {
                        data: 'collection_form_sku_detail.collection_form_sku.customer_sku.name'
                    },
                    {
                        data: 'collection_form_sku_detail.collection_form_sku.customer_sku.fabric_type'
                    },
                    {
                        data: 'machine_batch.machine.code'
                    },
                    {
                        data: 'machine_batch.machine.name'
                    },
                    {
                        data: 'collectionFormSkuDetail.weight'
                    },
                    {
                        data: 'collectionFormSkuDetail.created_at'
                    },
                    {
                        data: 'collectionFormSkuDetail.lastChild.created_at'
                    }
                ],
                columnDefs: [{
                    targets: '_all',

...
@section('content')
    <div class="dashboard-balancing">
        <div class="top-page">
            <a
                href="{{ route('backend.dashboard.index') . '?factory_id=' . request()->factory_id . '&date1=' . request()->date1 . '&date2' . request()->date2 }}"><img
                    src="{{ asset('images/icon/arrow-back.svg') }}" alt="Ironing"></a>
            <p>List / <span style="font-weight: 700">{{ ucwords($type) }}</span></p>
        </div>

        <div class="right-item" id="switch-master-ongoing">
            <a class="item {{ request()->machine_type === 'drying' ? 'active' : '' }}"
                href='{{ route("backend.dashboard.on-process.$type.index") . '?factory_id=' . request()->factory_id . '&date1=' . request()->date1 . '&date2=' . request()->date2 . '&machine_type=drying' }}'>Dryer</a>
            <a class="item {{ request()->machine_type === 'dryer_solax' ? 'active' : '' }}"
                href='{{ route("backend.dashboard.on-process.$type.index") . '?factory_id=' . request()->factory_id . '&date1=' . request()->date1 . '&date2=' . request()->date2 . '&machine_type=dryer_solax' }}'>Dryer
                Solax</a>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#table-master-user').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: `{{ route("backend.dashboard.on-process.$type.index") }}`,
                    type: 'GET',
                    data: function(d) {
                        ...
                        d.date2 = $('#date2').val();
                        d.type = 'ongoing';
                        d.machine_type = `{{ request()->machine_type }}`;
                    }
                },
                columns: [{
                        data: 'no'
